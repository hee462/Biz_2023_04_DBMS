...
SELECT acNum, acDiv,acBuId,acBanlance
FROM tbl_acc
ORDER BY acNum;
...
SELECT * 
FROM tbl_acc
WHERE acBanlance >= 10000 AND acBanlance <= 20000;
--  각각 범위 값에 포함관계(>=,<=)인 경우 
-- BETEEW 키워드로 검색가능
SELECT * 
FROM tbl_acc
WHERE acBanlance BETWEEN 10000 AND 20000;

--acBuId 에 0001 OR 0003의 반대인 경우
--acBuId 에 0001 도 아니고 0003도 아닌 값의 조건검색
SELECT *
FROM tbl_acc
...
--초기에 설계가 잘못된 경우 구조변경을 실시하는데 상당한 주의를 기울여야한다
--구조변경 과정에서 문제가 발생하면 데이터가 손상되는 문제가 있다.
--구조변경을 할때 칼럼의 크기는 절대 줄여서 변경하면 안된다.

-- tbl_acc의 acNum 칼럼의 길이가 VARCHARA2(10)인데 
-- 이 칼럼의 길이를 VARCHAR(20)으로 변경

ALTER TABLE tbl_acc MODIFY (acNum VARCHAR2(20));
DESC tbl_acc;
-- SQL 에는 기본적으로 여러 기능을 수행하는 함수들이 내장되어 있다
-- substr(칼럼,시작,개수) : 칼럼의 문자열을 시작 위치부터 개수만큼 잘라내기
-- max(칼럼) : 전체 리스트(조건에 맞는 데이처 중에서) 중에서 가장 큰 값을 찾기
-- min(칼럼) : 전체 리스트(조건에 맞는 데이처 중에서) 중에서 가장 작은값 찾기
