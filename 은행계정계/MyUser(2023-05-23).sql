...
전화번호가 없는 성춘향 데이터의 전화번호를 010-333-3333으로 변경하고자 한다

1.buname칼럼의 데이터가 성춘향인 리스트를 조회한다
2.전화번호가 없는 (null) 데이터의 buid를 확인한다 
3.buid을 기준으로 UPdate를 실시한다
...
/*
1.전체 고객 데이터를 조회
2.이몽룡의 주소가 현재 서울 특별시 이다
그런데 이몽룡이 전라북도 익산시로 이사를 했다
3.이몽룡의 주소를 서울특별시에서 전라북도 남원시로 변경하고자 한다

*/
SELECT *FROM tbl_buyer;

SELECT *FROM tbl_buyer WHERE buname ='임꺽정';
UPDATE tbl_buyer
SET buaddr ='전라북도 남원시'
WHERE buid ='0001';
...
INSERT INTO tbl_acc(acNum,acDiv, acBuId,acBanlance)
VALUES('2023052301','1','0003',10000);

INSERT INTO tbl_acc(acNum,acDiv, acBuId,acBanlance)
VALUES('2023052302','1','0001',5000);
...
/*
현재 계좌정보를 조회했는데
고객정보가 고객ID 뿐이여서 고객에 대한 이름,전화번호 등을 알수가 없다.
고객정보와 계좌정보를 연계하여 함께 볼수 있다면 좋겟네

Table JOIN
2개의 Table을 서로 연계하여 하나의 리스트로 보기

tbl_acc 와 tbl_buyer 테이블을 연계하여 하나의 리스트로 보여라
이때 tbl_acc의 acBuId와 tbl_buyer buId칼럼의 데이터를 비교하여 
같은 데이터는 한 라인으로 보여라
*/
...
SELECT * FROM tbl_buyer;
-- Projection : SELECT 조회를 할때 별표(*)를 사용하지 않고 칼럼을 나열하는것
...
VALUES('0004','변학도','010444444');

/*SQL Developer 와 java 코드에서 DB를 서로 연동하여 처리하는 경우 발생하는 문제
SQL 에서 데이터를 INSERT, UPDATE, DELETE 를 수행하는 경우
    추가, 수정, 삭제 된 정보는 아직 storyage 에 반영되지 않고,
    메모리에 임시 보관(저장) 된 상태이다
이 상태일때 java에서 SELECT 를 수행하면, INSERT ,UPDATE,DELETE 된 데이터가
아닌 이전의 상태의 데이터가 조회된다
간혹 이 상황에서 DBMS가 Connection에서 무한정 응답하지 않는 경우도 있다
java 는 DBMS가 응답하기를 기다리면서 무한정 기다리고 마치 
프로젝트가 멈춘(DOWN) 상태가 되어버린다

SQL 에서 INSERT ,UPDATE,DELETE를 수행한 다음에는 
강제로 storyage에 COMMIT해주어야 한다
...
-- COMMIT 되기 전에 데이터를 취소하는 명령
ROLLBACK;

--PK 칼럼을 기준으로 조건을 설정하여 조회하기
--PK 칼럼을 기준으로 조회하면 데이터가 없거나, 1개만 조회된다
SELECT buid,buname,butel,buaddr,bubirth,bujob
FROM tbl_buyer
WHERE buid ='0001';

INSERT INTO tbl_buyer(buid,buname,butel)
VALUES('0001','이이이','010-45554');
