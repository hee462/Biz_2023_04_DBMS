...
--iolist User가 접근(CRUD) 실행 할수 있는 table을 보여달라
SELECT* FROM all_tables WHERE owner ='IOLIST';
--tbl_product Table의 속성(Domain)을 보여달라
--DBMS에서 Domain :table의 속성(칼럼)의 데이터 type
 ...
 --PK 기준으로 조회 1개 가 조회되거나 NULL 값이다
 SELECT * FROM tbl_buyer WHERE buid ='0000000001';
  SELECT * FROM tbl_buyer WHERE buid ='0000000007';
  
  --PK 이외의 칼럼으로 기준으로 조회하면 비록 1개의 결과만 보일지라도
  -- 이 결과는 1개 이상인 것으로 본다.
  -- 따라서 이 결과는 empty(결과없음) 0이거나 1개 이상 즉 중복된 개체가 있을수 있음
