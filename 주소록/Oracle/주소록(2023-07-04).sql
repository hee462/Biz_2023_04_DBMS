...
--단순히 두테이블을 두고 ah_aid 와 a_id값이 동일한것만 나열하였다
SELECT *
FROM tbl_addr_hobby,tbl_address
WHERE ah_aid = a_id;

-- 두 테이블에서 ah_aid와 a_id가 동일한 값을 셀렉한 칼럼을 순서대로 나열함
SELECT ah_aid,a_name,a_tel,a_addr, ah_hbcode,a_id
FROM tbl_addr_hobby,tbl_address
WHERE ah_aid = a_id;

-- 두개의 테이블 ah_aid 와 a_id 에서 동일한지 비교하고 count 함
SELECT COUNT(*)
FROM tbl_addr_hobby,tbl_address
WHERE ah_aid = a_id;
