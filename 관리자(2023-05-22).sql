...
-- 삭제, 제거 : DELETE, DROP
-- DELETE :데이터의 일부요소를 삭제 하기, 데이터 구조 등은 변화가 없다
-- DROP : 물리적인 데이터 요소를 제거하기 , 데이터 구조 제거, 많은 데이터를 한꺼번에 제거
--        데이터 파과제 명령어, 매우 신중하게 주의하면서 사용해야 한다
--DROP TABLESPACE myDB
--INCLUDING CONTENTS AND DATAFILES;
--21c 에서 일부 권한이 필요한 스크립트 실행이 가능하도록 설정하기
...
REVOK CREATE TABLE FROM C##myuser;

-- 실습의 편리성을 위하여  SYSDBA 보다 한단계 낮은 일반 DBA 권한을 부여
-- 오라클에선는 SYSDBA(SYSTEM등)의 User 권한은 매우 조심해서 신중하게 다루어야 한다
-- DB 사용의 편의성을 위하여 오라클에서는 DBA권한을 일반 사용자에게 부여할수 있도록 하고있다

-- 이 권한은 실제환경에서는 절대 사용하면 안됀다
-- 시스템의 중요한 데이터 사전을 제외한 모든 데이터를 핸들링(추가,수정,삭제,제거 등)을 할수 있는 권한이다
-- 다만 실습 환경에서만 사용하자
